<template>
  <section class="page">
    ...
    <h3>使用</h3>
    <p>从 moto.js 里导入 chain 和 tween。</p>
    <pre><code class="js" v-text="snippets[0]"></code></pre>
    <p>让一个物体沿 x 轴移动后再沿 y 轴移动。</p>
    <pre><code class="js" v-text="snippets[1]"></code></pre>
    <i class="placeholder"></i>
    <h3>说明</h3>
    <p>chain 函数传入一个或者多个动画实例：</p>
    <ul>
      <li><i>...animes:</i><i>tween 或者 curve 下系列曲线函数的返回值</i></li>
    </ul>
    <p>执行 chain 函数会返回一个 chain&lt;object&gt;，包含如下方法：</p>
    <ul>
    ...
    <h3>演示</h3>
    <p>chain 效果演示。</p>
    <iframe width="100%" height="500" src="//jsfiddle.net/JetLu/c67zqh28/55/embedded/result,js/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>
  </section>
</template>
...
<script>
  ...
  export default {
    ...
    created() {
      import('./snippets/chain.txt').then(res => {
